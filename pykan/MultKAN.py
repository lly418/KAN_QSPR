...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity=2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0,
        ...
        for l in range(self.depth):
            # splines
            sp_batch = KANLayer(in_dim=width_in[l], out_dim=width_out[l + 1], num=grid, k=k, noise_scale=noise_scale,
                                scale_base_mu=scale_base_mu, scale_base_sigma=scale_base_sigma, scale_sp=1.,
                                base_fun=base_fun, grid_eps=grid_eps, grid_range=grid_range, sp_trainable=sp_trainable,
    ...
    def reg(self, reg_metric, lamb_l1, lamb_entropy, lamb_coef, lamb_coefdiff):
        ...
        if lamb_l1 + lamb_entropy + lamb_coef + lamb_coefdiff < 1e-4:
            reg_ = reg_ / (lamb_l1 + lamb_entropy + lamb_coef + lamb_coefdiff)
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
        ...
        for _ in pbar:
            ...
            results['reg'].append((lamb * reg_).cpu().detach().numpy())

            # if _ % log == 0:
            # if display_metrics == None:
            # print(f"| train_loss: {torch.sqrt(train_loss).cpu().detach().numpy():.2f} | reg: {reg_.cpu().detach().numpy():.2f} | ")
    ...
    def suggest_symbolic(self, l, i, j, a_range=(-10, 10), b_range=(-10, 10), lib=None, topk=5, verbose=True,
                         r2_loss_fun=lambda x: np.log(1 + 1e-5 - x), c_loss_fun=lambda x: np.exp(x), alpha=0.1,
                         beta=1.5):
        '''
        ...
        r2s = np.array(r2s)
        cs = np.array(cs)
        r2_loss = beta * r2_loss_fun(r2s).astype('float')
        cs_loss = c_loss_fun(alpha * cs)

        loss = cs_loss + r2_loss
    ...
    def auto_symbolic(self, a_range=(-10, 10), b_range=(-10, 10), lib=None, verbose=1, alpha=0.1, beta=1.5,
                      r2_threshold=0.0):
        '''
                ...
                for j in range(self.width_out[l + 1]):
                    if self.symbolic_fun[l].mask[j, i] > 0. and self.act_fun[l].mask[i][j] == 0.:
                        ...
                        name, fun, r2, c = self.suggest_symbolic(l, i, j, a_range=a_range, b_range=b_range, lib=lib,
                                                                 verbose=False, alpha=alpha, beta=beta)
                        if r2 >= r2_threshold:
